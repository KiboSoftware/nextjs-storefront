...
describe('[component] - OrderStatusTemplate', () => {
  it('should view order status by providing the order number and billing email', async () => {
    ...
    expect(screen.queryByText(/order-number/i)).toBeVisible()
    expect(screen.queryByText(orderNumber as number)).toBeVisible()
    expect(screen.queryByText(/order-date/i)).toBeVisible()
    expect(screen.queryByText(submittedDate)).toBeVisible()
    expect(screen.queryByText(/shipped-to/i)).toBeVisible()
    expect(screen.queryByText(shippedTo)).toBeVisible()
  })
})
