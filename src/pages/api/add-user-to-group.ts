...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const addUserToGroupResponse: any = await gqlFetch(
      {
    ...
    if (addUserToGroupResponse.ok) {
      ...
      return res.status(addUserToGroupResponse.status).json(result)
    }
  } catch (error) {
