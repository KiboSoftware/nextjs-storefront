...
export default async function addPaymentHandler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const resp: any = await fetch(
      addProtocolToHost(process.env.KIBO_API_HOST) +
        `/api/commerce/orders/${variables.orderId}/payments/actions`,
      {
        method: 'POST',
        headers: {
    ...
    res.status(200).json({ data: { createOrderPaymentAction: response }, status: 200 })
  } catch (error: any) {
