...
export default async function getSubscriptionsHandler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const resp: any = await fetch(
      addProtocolToHost(process.env.KIBO_API_HOST) + `/api/commerce/subscriptions`,
      {
        method: 'GET',
        headers: {
    ...
    res.status(200).json({ data: { subscriptions: response }, status: 200 })
  } catch (error: any) {
