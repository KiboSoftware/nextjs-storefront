...
describe('[components] OrderPriceCollapsible', () => {
  ...
  it('should render component with discount', async () => {
    ...
    expect(screen.getByText(WithDiscounts.args?.orderDiscounts?.[0].name as number)).toBeVisible()
    expect(screen.getByText(WithDiscounts.args?.orderDiscounts?.[0].impact as number)).toBeVisible()
