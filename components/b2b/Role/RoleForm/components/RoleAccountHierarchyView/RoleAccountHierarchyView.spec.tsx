...
describe('[Component] RoleAccountHierarchyView', () => {
  ...
  describe('System Role Functionality', () => {
    it('should display system role message when isSystemRole is true', () => {
      const { getByText, queryByRole } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000, 1001, 1002]}
          parentAccountId={1000}
          isSystemRole={true}
        />
      )

      expect(getByText('system-role-applies-to-all-accounts')).toBeInTheDocument()
      expect(queryByRole('checkbox')).not.toBeInTheDocument()
    })

    it('should not display account hierarchy tree for system roles', () => {
      ...
      expect(queryByText(/Child Company 1/)).not.toBeInTheDocument()
    })

    it('should not display account count for system roles', () => {
      const { queryByText } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000, 1001, 1002]}
      ...
      expect(queryByText(/Role will be applied to/)).not.toBeInTheDocument()
    })

    it('should toggle between system role and custom role views', () => {
      ...
      // Custom role should show hierarchy
      expect(getByText(/Parent Company/)).toBeInTheDocument()
      expect(queryByText('system-role-applies-to-all-accounts')).not.toBeInTheDocument()
      ...
      // System role should show message
      expect(getByText('system-role-applies-to-all-accounts')).toBeInTheDocument()
      expect(queryByText(/Parent Company/)).not.toBeInTheDocument()
    })

    it('should render system role message with proper styling', () => {
      ...
      const messageElement = getByText('system-role-applies-to-all-accounts')
      expect(messageElement).toBeInTheDocument()
      expect(messageElement.tagName).toBe('P')
    })

    it('should not render expand/collapse buttons for system roles', () => {
    ...
    it('should show only title and system message for system roles', () => {
      ...
      expect(getByText('Account Hierarchy Scope')).toBeInTheDocument()
      expect(getByText('system-role-applies-to-all-accounts')).toBeInTheDocument()
      expect(queryByRole('checkbox')).not.toBeInTheDocument()
    ...
    it('should handle system role with empty accounts', () => {
      const { getByText } = render(
        <RoleAccountHierarchyView
          accounts={[]}
          selectedAccountIds={[]}
      ...
      expect(getByText('No accounts available')).toBeInTheDocument()
    })

    it('should handle system role without parentAccountId', () => {
      const { getByText } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000]}
    ...
    it('System Role Scenario: Admin viewing all permissions', () => {
      const { getByText, queryByRole } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000, 1001, 1002, 1003]}
          parentAccountId={1000}
          isSystemRole={true}
        />
      )
    ...
    it('System Role Scenario: Purchaser with limited scope', () => {
      const { getByText } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000]}
    ...
    it('should differentiate between system and custom roles', () => {
      const { rerender, getByText, queryByText, queryByRole } = render(
        <RoleAccountHierarchyView
          accounts={mockAccounts}
          selectedAccountIds={[1000, 1001]}
          parentAccountId={1000}
      ...
      // Custom role displays hierarchy
      expect(getByText(/Parent Company/)).toBeInTheDocument()
      ...
      // System role displays message only
      expect(getByText('system-role-applies-to-all-accounts')).toBeInTheDocument()
      expect(queryByText(/Parent Company/)).not.toBeInTheDocument()
      expect(queryByRole('checkbox')).not.toBeInTheDocument()
    })
