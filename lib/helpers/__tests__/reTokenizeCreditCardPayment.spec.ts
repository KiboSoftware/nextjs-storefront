...
import { reTokenizeCreditCardPayment } from '../reTokenizeCreditCardPayment'
...
describe('tokenizeCreditCardPayment', () => {
  it('should return the tokenized credit card data if the API request is successful', async () => {
    ...
    // Act
    const tokenizedCCData = await reTokenizeCreditCardPayment(creditCardData, pciHost, apiHost)
