...
describe('[getters] productGetters', () => {
  ...
  it('should return the product coverImage', () => {
    expect(productGetters.getCoverImage(ProductDataMock)).toBe(
      ProductDataMock?.content?.productImages[0].imageUrl
    )
  })

  it('should return the product id', () => {
    expect(productGetters.getProductId(ProductDataMock)).toBe(ProductDataMock?.productCode)
  })

  it('should return the product variation product code or code', () => {
    expect(productGetters.getVariationProductCodeOrProductCode(ProductDataMock)).toBe(
      ProductDataMock?.productCode
    )
  })
