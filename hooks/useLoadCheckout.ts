...
export const useLoadCheckout = (checkoutId: string): UseLoadCheckout => {
  const { data, isLoading } = useQuery([querykeys.LOAD_CHECKOUT, checkoutId], () =>
...
export const useLoadFromCart = (cartId: string): UseLoadFromCart => {
  const { data = [], isLoading } = useQuery([querykeys.LOAD_FROM_CART, cartId], () =>
    getOrCreateCheckout(null, cartId)
  )
  return { data, isLoading }
}
